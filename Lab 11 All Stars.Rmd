...
johnny_full <- babynames_all %>%
 filter(str_detect(name, "^Jo[ah]?[n]+ie[y]*$")) %>%
  group_by(year) %>%
...
Using the regexs "A[r]+[iye][ea][l][le]*$" for Ariel and "^R[ae][aey]*[c]h[aey]+[l]+e?$" for Rachel,
there are `r arielrachelversions` different versions of each name for each year, and the chances a girl 
would have a version of the name Ariel or Rachel in each year is `r arielrachelprop`.
